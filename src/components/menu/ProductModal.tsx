  ...
  const getPriceForContext = (targetSize: Size, targetPowderId: string, milkId?: string) => {
    ...
    for (const g of item.addon_groups) {
      if (g.type === "QUANTITY") {
        ...
        if (qty > 0 && opt) addonsCost += qty * (opt.gram_value != null ? opt.gram_value * pwd_price_per_gram : opt.price_vnd);
      } else {
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="px-5 pb-44">
          ...
          {/* 4. COLDWHISK - Now an OptionCard, directly under Milk/Powder */}
          <div className="mt-7">
